...
public class KMDBOperation {
	...
	public static void DeleteWordByID(String id) {
		try {
			...
			if (Database.executeNonQuery(sql) == -1) {
				GlobalValue.myLog.error(">>sql:" + sql);
			}
		} catch (Exception e) {
			GlobalValue.myLog.error(e.toString());
		}
	...
	public static String GetWordClassIDByWordClass(String wordClass) {
		try {
			...
			return rst.getRows()[0].get("wordclassid").toString().trim();
		} catch (Exception e) {
			GlobalValue.myLog.error(e.toString());
			return "";
		}
	...
	public static void CreateOrReplaceSeqWordID() {
		...
		if (maxWordID.equals("")) {
			GlobalValue.myLog.error(">>Fail to CreateOrReplaceSeqWordID!");
			return;
		}
	...
	public static int AddOtherName4WordID(String wordID,
		...
		try {
			...
			String wordClassID = rst.getRows()[0].get("wordclassid").toString()
					.trim();
			// 查出已有的别名
			...
			for (SortedMap<Object, Object> row : synWordTable.getRows()) {
			...
			if (Database.ExecuteSQL(sqlList) == false) {
				GlobalValue.myLog.error(">>插入别名出错：" + wordID + "_" + otherName
						+ "_" + wordClassID);
				return 0;
			}
			return 1;
	...
	public static int UpdateWord(String word, String otherNames,
		...
		try {
			...
			if (rst == null || rst.getRowCount() == 0) {
				GlobalValue.myLog.error(">>需要批量导入的词类：" + wordClass + "不存在！"
						+ sql0);
				return 0;
			}
			String wordClassId = rst.getRows()[0].get("wordclassid").toString()
			...
			if (rst == null || rst.getRowCount() == 0) // 不存在该词，进行插入
			{
				...
				String wordid = rst.getRows()[0].get("wordid").toString();
				// 查出已有的别名
				String sql = "Select x.wordid, x.wordclassid wordclassid, wordclass, x.word, x.type, y.word stdword";
				sql += " from Word x, word y, WordClass";
				sql += " where WordClass.WordClassID = x.WordClassID";
			...
			// 执行sql集
			if (Database.ExecuteSQL(sqlList) == false) {
				GlobalValue.myLog.error(">>数据批量导入出错：" + word + "_" + otherNames
						+ "_" + wordClass + "_" + operationType);
				return 0;
			}
			return wordCNT;
		} catch (Exception e) {
	...
	public static void updateWord(ArrayList<String> wordList, String wordClass,
			String wordclassid ,User user) {
		// 读出该词类的所有词条，包括别名
		Map<String, String> wordMap = new HashMap<String, String>();
		String word = "";
		Result rs = CommonLibWordDAO.select(null, null, true, null, wordClass,
				"基础");
		if (rs != null && rs.getRowCount() > 0) {
			for (SortedMap<Object, Object> row : rs.getRows()) {
				word = DBValueOper.GetValidateStringObj4Null(row.get("word"));
				if (word.length() > 0) {
					wordMap.put(word, "1");
				}
			}
		...
		if (wordMap.size() <= 0) {
			System.out.println("词类词条为空");
		}
		for (String ss : wordList) {
			if (!wordMap.containsKey(ss)) {
				
				CommonLibWordDAO.insert(ss, wordclassid, user);
			} else {
